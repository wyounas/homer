...
class Word(object):
    ...
    def __init__(self, word):
        ...
        self.is_abstract_noun = True if any([self.word.startswith(abs_noun) for abs_noun in abs_nouns]) else False
        # Check if this word is a zombie noun
        zombies = ['ance', 'ment', 'ments', 'ation', 'ations', 'ing', 'ty']
        # From world-bank paper: https://litlab.stanford.edu/LiteraryLabPamphlet9.pdf
...
class Sentence(object):
    ...
    def __init__(self, sentence):
        ...
        self.abstract_nouns = (word for word in self.words if word.is_abstract_noun)
    ...
    def get_zombie_nouns(self):
        """Get zombie nouns in a sentence. """
...
class Paragraph(object):
    ...
    def __init__(self, paragraph):
        ...
        self.compulsive_hedgers = list(itertools.chain(*[sentence.compulsive_hedgers for sentence in self.sentences if sentence.compulsive_hedgers]))
        self.intensifiers = list(itertools.chain(*[sentence.intensifiers for sentence in self.sentences if sentence.intensifiers]))
        self.abstract_nouns = list(itertools.chain(*[sentence.abstract_nouns for sentence in self.sentences if sentence.abstract_nouns]))
        self.zombie_nouns = list(itertools.chain(*[sentence.zombie_nouns for sentence in self.sentences if sentence.zombie_nouns]))
        self.vague_words = list(itertools.chain(*[sentence.vague_words for sentence in self.sentences if sentence.vague_words]))
        ...
        # TODO: a)- highlight longer sentences b)- highlight long paras

    def __repr__(self):
        return "sentences= {sentences}, flesch_reading_score={flesch_reading}, dale_chall={dale_chall}".format(
            sentences=self.total_sentences, flesch_reading=self.flesch_reading_score, dale_chall=self.dale_chall_readability_score
        )
class Article(object):
    """
    An article represents a block of tests consisting of paragraphs. For example, it could be an essay or a blog post.
    ...
    def __init__(self, name, author, text):
        ...
        self.read_time = self.total_words / 200 # gets us minutes (since we read 200 words per minute so 600 / 200 is 3 minutes of reading time)
        self.avg_sentences_per_para = round(self.total_sentences / self.total_paragrpahs, 2)
        self.avg_words_per_sentence = round(self.total_words / self.total_sentences, 2)
        # Flattening list of lists
        self.zombie_nouns = list(itertools.chain(*(paragraph.zombie_nouns for paragraph in self.paragraphs if paragraph.zombie_nouns)))
        self.total_zombie_nouns = len(self.zombie_nouns)
        self.compulsive_hedgers = list(itertools.chain(*(paragraph.compulsive_hedgers for paragraph in self.paragraphs if paragraph.compulsive_hedgers)))
        self.total_compulsive_hedgers = len(self.compulsive_hedgers)
        self.intensifiers = list(itertools.chain(*(paragraph.intensifiers for paragraph in self.paragraphs if paragraph.intensifiers)))
        self.total_intensifiers = len(self.intensifiers)
        self.abstract_nouns = list(itertools.chain(*(paragraph.abstract_nouns for paragraph in self.paragraphs if paragraph.abstract_nouns)))
        self.total_abstract_nouns = len(self.abstract_nouns)
        self.vague_words = list(itertools.chain(*(paragraph.vague_words for paragraph in self.paragraphs if paragraph.vague_words)))
        ...
        print("Article score")
        print("-------")
        print(self.dale_chall_readability_score)

    def words_with_most_syllables(self):
    ...
    def print_paragraph_stats(self):
        ...
        for item, para in enumerate(self.paragraphs):
            ...
            table_data.append([item + 1,
                               '{sentences} {sent_tag}. {words} {word_tag}. {avg_words} {avg_word_tag}. "{longest_sent}..." is the {long_tag} sentence.'.format(
    ...
    def print_article_stats(self):
        ...
        table_data = [
            ...
            ['Abstract Nouns', self.total_abstract_nouns],
    ...
    def print_detail(self):
        ...
        if len(self.intensifiers) >= 1:
            msg = '{red} **- Intensifiers: %s {/red}\r\n' % ', '.join(str(intensifier) for intensifier in self.intensifiers)
            print(Color(msg))

        if len(self.abstract_nouns) >= 1:
            msg = '{red} **- Abstract nouns: %s {/red}\r\n' % ', '.join(str(abs_noun) for abs_noun in self.abstract_nouns)
            print(Color(msg))
        ...
        words_with_most_syllables = self.words_with_most_syllables()
        if len(words_with_most_syllables) >= 1:
            msg = '{red} **- 10 words with most syllables: %s {/red}\r\n' % ', '.join(words_with_most_syllables)
            print(Color(msg))
...
article = Article('name', 'author', open('/Users/waqas/PycharmProjects/sense_of_style/experiment/oped.txt').read())
